...
- **Merging a PR: use `scripts/merge-pr.sh <number>`**, not `gh pr merge`. CodeRabbit re-reviews on every push, so an "0 unresolved threads" reading taken before the latest push is stale and reads exactly like all-clear. The script refuses to merge unless the newest review is newer than the newest commit. It also *requests* a review when the PR has moved since its last one, because automatic incremental reviews are disabled in `.coderabbit.yaml` — review runs are capped per hour, and re-reviewing every push spends that budget on intermediate states nobody merges. `--force` overrides deliberately. `scripts/pr-review-status.py` is the same check on its own; don't pipe it if you care about the exit code.
