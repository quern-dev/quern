...
GETWEBPROXY_DISABLED = "Enabled: No\nServer: \nPort: 0\nAuthenticated Proxy Enabled: 0\n"

GETSECUREWEBPROXY_ENABLED = (
    "Enabled: Yes\nServer: 192.168.1.100\nPort: 8443\nAuthenticated Proxy Enabled: 0\n"
)

GETSECUREWEBPROXY_DISABLED = "Enabled: No\nServer: \nPort: 0\nAuthenticated Proxy Enabled: 0\n"
...
class TestDetectAndConfigure:
    def test_success(self):
        with (
            patch("server.proxy.system_proxy.detect_active_interface", return_value="Wi-Fi"),
    ...
    def test_interface_override(self):
        with (
            patch("server.proxy.system_proxy.detect_active_interface") as mock_detect,
            patch("server.proxy.system_proxy.snapshot_system_proxy") as mock_snap,
...
class TestRestoreFromState:
    ...
    def test_not_configured(self):
        with patch(
            "server.lifecycle.state.read_state", return_value={"system_proxy_configured": False}
        ):
            assert restore_from_state() is False
    ...
    def test_configured_with_snapshot(self):
        ...
        with (
            patch("server.lifecycle.state.read_state", return_value=state),
...
class TestStartProxySystemProxy:
    ...
    def test_start_with_system_proxy_false(self, client, mock_proxy_adapter):
        ...
        with (
            patch("server.api.proxy.detect_and_configure") as mock_detect,
            patch("server.api.proxy.update_state"),
        ):
...
class TestConfigureSystemEndpoint:
    ...
    def test_configure_already_configured(self, client, mock_proxy_adapter):
        with patch(
            "server.lifecycle.state.read_state", return_value={"system_proxy_configured": True}
        ):
            resp = client.post("/api/v1/proxy/configure-system")
