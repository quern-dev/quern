...
- **`scripts/install-menubar-app.sh`** — installs the menu-bar app on a git-clone install, which `quern setup` deliberately skips. Together with start-on-launch it is the whole setup: one command, after which the CLI is optional. It refuses to install without `~/.local/bin/quern`, before downloading anything, since the app drives the daemon through that wrapper and would otherwise install cleanly and then fail at the first thing it tried. `--skip-wrapper-check` overrides it. Defaults to the signed, notarized app from the release matching your checkout; `--build` compiles from source instead. Both install to `~/Applications` and quit a running copy first. The guide previously told clone users the app "arrives with Quern", which is true for every install except theirs.

### Changed
- **`quern doctor` no longer needs a running server.** Only the device-tool section did, and doctor exited at the first check it could not make, printing nothing else. That withheld its most useful output — whether the venv matches `pyproject.toml` — behind exactly the condition that sends people looking for it, since a stale venv is a good way to stop the server coming up. It now names why the device-tool section is unavailable and reports the venv, external tools and service health regardless. The exit status for that case is unchanged: a check that could not run is not one that passed.

### Fixed
- **The menu-bar app's "Restart to Update" works again.** It watched for a `pyproject.toml` next to its own bundle, which was correct while `Quern.app` lived inside the install tree and became permanently wrong in v0.16.0 when the app moved to `~/Applications`. The read failed on every poll, so the version never appeared to change, and every update ran the full three-minute timeout before reporting one. It now asks `quern --version`, which has no path to get wrong.
- **`quern start` exits nonzero when the health check times out.** It printed a warning to stderr and exited 0, telling every caller the opposite of what happened. The menu-bar app gates its error reporting on that status, so a server that never became healthy produced a dimmed icon, a "stopped" menu and no explanation anywhere — indistinguishable from never having tried. The child process is deliberately left running, since it may still be coming up and its logs say why — so the menu-bar app waits and re-checks before reporting a failed start, rather than raising an alert that a healthy menu contradicts fifteen seconds later. While it waits the menu says "Starting…" and withdraws the lifecycle items, since `quern start` blocks for up to thirty seconds and a second one races the first for the port. When it does give up it offers **Open Server Log**, which is where the reason is, and the menu-bar icon turns red — dimming alone cannot distinguish "nobody started it" from "starting it failed", and the icon is the only part most people look at.
- **The menu bar no longer trusts a recycled process ID.** A stale `state.json` surviving a hard power-off would match an unrelated process after reboot, and the menu would report Quern running with an uptime describing something else. It now also checks that the process did not start after the state describing it was written.
- **The menu bar reports failures instead of doing nothing.** Start, Stop and Restart discarded the CLI's exit status, so a failure left the menu saying exactly what it said before and the click read as a dead menu item. They now raise the same alert "Quit and Stop Server" already used.
- **The menu bar says so when it cannot find the `quern` command.** It fell back to `/usr/bin/env quern` and let that fail with 127, which is indistinguishable from the command running and failing. A GUI app does not inherit your shell's PATH, so a CLI reachable from your terminal may well be invisible to it. It now names the wrapper it looked for and says to run setup.
...
### Fixed
- **v0.15.0 reached existing users without the menu-bar app** — the updater they ran from predates the release-asset preference, so it fetched GitHub's generated source tarball, which is source only. The code that prefers the asset shipped *inside* that asset, and so could not help itself. Nothing said so either: the update reported success and setup reported all checks passed, with the app simply absent. `quern setup` now fetches it when a release install is missing it, which is the first code of ours that runs on an affected machine — `quern update` sees the latest version already installed and downloads nothing. A git checkout is left alone, and a failure warns with the manual route rather than ending setup.
- **The active device shows its name again after a restart** — the sidecar is written by a setter on the device controller, and a restored device never ran it: startup assigns the persisted UDID straight to the backing field, and the device pool's sticky-active path returns that UDID without assigning it. A session that never resolved a device explicitly by UDID or name therefore showed a bare 36-character identifier for its whole life. The sidecar is refreshed once at startup, after the device caches are warm.
- **`quern help` works, and the help lists every command** — typing the most obvious thing in the tool exited 2 with an "invalid choice" error, because argparse only understands `-h`/`--help`. The usage line also named `__main__.py` rather than `quern`, since the wrapper execs a module. Worse, six real commands were missing from the help entirely — including `update` — because they are dispatched before argparse sees them, so the parser did not know they existed.
- **Doctor says which part of the tunneld plist is outdated** — the check tests the log path and the recorded `pymobiledevice3` binary, but reported the log path whichever one failed. A machine whose binary had drifted was told its log path was stale, reinstalled, and saw the same message, because the reinstall corrected a path that was already correct.

## [0.15.0] - 2026-09-10

### Added
- **A menu-bar app** — Quern now ships a signed, notarized `Quern.app` that lives in the menu bar, for the times you want to see whether the server is up without opening a terminal. It reports the daemon's state and uptime, the active device, and the proxy's port; it can start, stop and restart the server, and offers "Restart to Update" once an update is staged, the way Ollama does. A Settings window carries the update channel picker and a launch-at-login toggle. It owns nothing: the daemon's lifecycle is still the CLI's, and the app only ever asks. Attached to this release as `quern-0.15.0.tar.gz`, which bundles the app alongside the source tree — the updater prefers that asset, so an update brings the app with it.
- **The active device is named, and says what kind it is** — the menu bar reads the active device from a sidecar file the server writes, which previously held only a UDID. A 36-character identifier is not something a person recognises, and it set the width of the entire dropdown. The server now records the device's name and type alongside it, so the row reads `iPhone 16 Pro (Simulator)`, `iPhone 11 (Device)` or `Pixel 7 (Emulator)`. An unknown type is written as absent rather than guessed, because labelling real hardware a simulator is worse than saying nothing.
- **The screen-mirror app is built during setup** — it was compiled the first time something asked for a preview, which meant the menu bar's "Screen Mirror…" item was missing on a fresh install until you had already driven a preview from a terminal. The feature was hidden from exactly the people the menu bar exists for. It is built during `quern setup` now, about 1.5 seconds, and treated as optional the way scrcpy is: without Xcode Command Line Tools the check is skipped with `xcode-select --install` as the fix, and an offer to open the installer.
- **CI compiles the menu-bar app** — nothing built the Swift app, so a syntax error in it would pass every check and surface only when a maintainer ran the build by hand. The new job builds universal, matching what a release ships, verifies the bundle is well formed, and treats warnings as errors.
- **A wedged tunneld now heals itself** — a failed device pairing can leave the daemon alive holding no listener, which launchd reports as perfectly healthy, so `KeepAlive` never fires and it stays broken until someone thinks to suspect it (#73). 0.15.0-beta.1 learned to *recognise* that state; it could only print a remedy for a human to run. Quern now recovers it: the plist sets `KeepAlive`, so getting the stuck process to exit is the entire fix — launchd respawns it against clean state in about a second. Measured on a real wedge (pid 949, alive, no listener → respawned and serving) and on an injected one (`SIGSTOP`, detected as `wedged` and healed with no interaction). This is the same shape as the accessibility-bridge reset in #66; the analogy was thought not to hold here because recovery needs root, and the missing piece was a privilege narrow enough to grant.
- **`quern tunneld grant-recovery` / `revoke-recovery`** — recovery needs root, and prompting inside an unrelated operation is not an option: the caller is usually a script, and a blocked password prompt is indistinguishable from a hang. So the authorisation is taken once, explicitly, and it is deliberately tiny — one `NOPASSWD` rule for `launchctl kill SIGKILL system/com.quern.tunneld`, fully specified, no wildcards. It permits one signal to one job, and nothing else: not loading, unloading or reconfiguring daemons. The file is validated with `visudo -c` **before** installation, since a malformed drop-in in `/etc/sudoers.d` breaks `sudo` for everything on the machine. Without the grant, behaviour is unchanged — the wedge is reported, not healed.

### Fixed
- **Screen mirror no longer opens a window for your phone's camera** — a connected iPhone publishes its rear camera as a second capture device, so the app opened a second window showing a black rectangle. The two are distinguishable, but not by the obvious route: a Continuity Camera arrives as a plain external device, and what separates it from a screen is that a screen reports the generic `iOS Device` model.
- **Screen-mirror windows fit the device** — the window opened at a fixed size roughly a tenth wider than any modern iPhone, and the difference was letterboxed into black bars down each side. A capture device reports no dimensions until it is actually streaming, so the window now waits for the stream and matches it, then holds that ratio through any resize.
- **Unplugging a phone closes its window** — the window survived its device, showing a frozen last frame and holding a session bound to something that no longer existed, which also meant replugging could not attach because the name was still taken. Windows now close on detach and open on attach, honouring whatever filter the app was started with.
- **The proxy and update state the menu bar reads can be trusted** — `proxy_status` could report a port that nothing was listening on, and the update check could answer the ahead-of-channel question by fetching, which made a read of local state depend on the network.
- **A stale update check can no longer resurrect a cleared cache** — switching channels clears the cached update result, but a check already in flight would write its answer afterwards, restoring the very state the switch had just discarded. Channel switching and cache invalidation are now coordinated across processes, and the guard has a test that fails when the lock is removed.
- **The updater installs the asset belonging to the release** — it matched `quern-*.tar.gz` and took whichever asset GitHub listed first, so a release carrying a stale archive could install that one while reporting the current tag as the version. It now requires the exact `quern-<version>.tar.gz`.
- **AVD names read as names** — an Android emulator reported `Pixel_7`, because AVD names cannot contain spaces. The underscores are a naming rule rather than anyone's choice, and they are stripped on the way to the menu bar. Simulator and physical-device names are untouched, since an underscore there was typed by a person.
- **`quern tunneld restart` no longer uses `kickstart -k`** — that sends `SIGKILL` and hung `launchctl` on macOS 15, which `install_daemon` has warned about in a comment since it was written. The warning sat four hundred lines from `_restart_daemon`, which used the forbidden call, so the one command anybody runs when tunneld looks wrong was the one command documented as dangerous. Restart now signals a loaded job and falls back to `bootout` + `bootstrap` when there is nothing loaded to signal, and the settle-and-retry logic both paths need is shared rather than duplicated — they had already drifted once, and that gap is how `kickstart -k` got in.
- **`quern tunneld restart` reports serving, not exit status** — it returned success whenever `launchctl` returned 0. The failure it exists to clear is a daemon launchd *considers healthy*, so an exit code is precisely the wrong evidence, and a restart that fixed nothing reported success. It now confirms the daemon answers on its port before saying so.
- **A failed screenshot names a wedged tunneld as the likely cause** — with no tunnel available, `Pmd3Backend.screenshot` silently drops to the usbmuxd path, which is right for iOS 16 and earlier and wrong for everything since. The failure surfaced pymobiledevice3's downstream error and never the reason. It now distinguishes "tunneld is not serving, and it may be wedged" from "tunneld is fine but has no tunnel for this device" — different problems pointing at different fixes — and logs a warning rather than an `info` when the fallback happens because the daemon is down.
...
- **sim-bridge no longer queues abandoned work without bound** (#68) — every command is serialised through one lock, and uvicorn does not cancel a handler when the HTTP client disconnects: a request abandoned after 0.4s was measured completing its full 1896ms of work server-side. So an abandoned request kept its slot and ran to completion, and a retry loop turned into a multi-minute outage — `describe_all` calls completing at **182 seconds** while returning perfectly healthy 6-element trees. The device looked hung and was not, and the symptom pointed nowhere near the cause. Concurrent *operations* are now bounded (`MAX_CONCURRENT_OPERATIONS`), with a saturation error that says the device is responsive and to stop retrying, surfaced as HTTP 503 rather than a generic 500. On the same 40-request repro the worst completion fell from 182s to 11.3s and the server recovered immediately instead of wedging.
- **A timed-out sim-bridge command can no longer hand its response to the next command** (#69 review) — the wire protocol carries no request IDs, and `_dispatch` resolves whichever `_pending_response` future happens to be current. So when a command timed out at 30s and released the lock, the next command installed its own future, and the late response from the *previous* command resolved it. The caller got a well-formed reply to a request it never made — a UI tree for a screen it never asked about — with nothing in the payload to reveal the substitution. Reproduced directly: command B received `{'belongs_to': 'COMMAND-A'}`. The subprocess is now terminated before the lock is released, which is the only sound remedy while responses are uncorrelated; it respawns lazily on the next command. The timeout message also now says the command was already sent, so the outcome is ambiguous and state-changing commands must not be retried automatically.
- **Python dependencies now reconcile themselves** — a new declared dependency could silently fail to install, leaving a server that starts happily and then fails at the point of use. `quern update`'s `pip install` failure was only a warning, so the update reported success onto a stale venv; and it skipped the reinstall entirely when the workspace was not pullable (the feature-branch case from #40). Neither path saw a manual `git pull` or a branch switch at all. `_ensure_python_deps()` now runs on every server start, mirroring how `_ensure_mcp_built` already handles `node_modules`: an mtime stamp at `.venv/.deps-stamp` decides whether work is needed, so the common case costs two `stat()` calls and no network. The stamp is written **only after a successful install**, so a failed one is never remembered as done and every subsequent start retries the install by itself. Recovery from a transient failure — unreachable packages, a network drop mid-install — needs no user intervention. A failure with a persistent cause, such as an invalid requirement, an unsatisfiable version constraint, or a missing build toolchain, will keep failing on each retry and still needs fixing; what the stamp guarantees is that the retry happens, not that it succeeds. The updater's separate `pip install` is gone; it delegates to the same helper, so there is one implementation and one failure semantic.
...
- **API reference extracted to `docs/api-reference.md`** — the endpoint tables were 165 of the README's 508 lines, and a parallel tool-to-endpoint mapping in `docs/agent-guide.md` covered only 65 of 107 tools. Both are now one generated document: every tool with the endpoint behind it, plus the 19 routes that have no MCP tool (SSE streams, public probes, CLI-only operations) which the agent guide never mentioned at all. The mapping is derived from each tool's own `apiRequest` call rather than maintained by hand. README drops to 360 lines and keeps its MCP tool table; the agent guide keeps a short list of common paths and points at the reference.
- **`quern://api-reference` registered as an MCP resource** — `docs/agent-guide.md` is served over MCP as `quern://guide`, so an agent reading it cannot follow a relative link out to a file on disk. The full reference is now addressable the same way.
- **`docs/guides/app-state.md` no longer contradicts the shipped feature** — it listed keychain items under "What's NOT included" and stated that credentials "won't be captured", while opening with `logged in with test data` as its headline example. It now documents `include_keychain`, the shut-down-device requirement and why it exists, and the quiet way the failure presents when the keychain is missing.
- **`docs/release-channels.md` stable-cut procedure advances every channel branch** — it only ever showed `release/stable`, which is what stranded `release/beta` 27 commits behind on the v0.14.0 cut, unfixably, because the release commit was already published. The procedure now advances both, adds a verification step (the rejection is silent), explains why beta must not lag stable, and notes that a prerelease advances beta only. The "why are both branches at main HEAD" section described a bootstrapping state that no longer exists and has been replaced.
- **Stale tool count corrected in `docs/linux-support-plan.md`** — "All 76 tools" → 107.
- **`scroll_to_element` and `quern doctor` documented in the agent guide** — both shipped in 0.14.0 and appeared nowhere in `docs/`. The guide now covers reaching off-screen elements (including why both scroll paths avoid a UI-tree dump) and points at `doctor` / `GET /tools` for tool availability, with an explicit note not to expect `/health` to answer that.
