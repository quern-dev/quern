...
- label_prefix: prefix match (case-insensitive) — useful when the label starts with a stable string but has variable content after`,
  ...
  server.registerTool("scroll_to_element", {
    description: `Scroll a scrollable container until an element is in view, WITHOUT tapping it. Resolve by identifier or label. Supported on Android and iOS (simulator + physical). Drives a bounded swipe loop that re-checks the target by selector after each swipe — no full UI-tree dump, so it avoids the dump-induced scroll a tree read can cause. The first lookup is the exception: it reads containers the static tree omits, so a target such as a tab-bar item is found without scrolling at all. Each swipe settles before the screen is read, because a mid-fling read can catch a row in flight and then lose it. Android covers View RecyclerView, Compose LazyColumn, and Compose-in-ScrollView. iOS handles both scroller shapes: a directional swipe toward a located-but-off-screen target in laid-out ScrollViews, and a blind down-then-up sweep for lazy/recycled lists where rows drop out of the tree; rows scrolled under the nav/status bar are rejected rather than counted as already visible. Bounded by a wall-clock deadline as well as the swipe budget, since a swipe count is not a time bound. Returns the element's on-screen position once visible; 404 if it never appears within the budget; 499 if the caller disconnects, which is not retryable — nothing is listening.`,
    inputSchema: strictParams({
      label: z
        .string()
        .optional()
