> **Archived, 2026-09-20. Not a plan.** Written 2026-02-14, the same day the
> hardcoded fast path it proposes replacing was added (`7a15719`). It was filed
> as "Phase 5" and never built; the shortcut outlived it by seven months and a
> public release, and was removed in #239 after a user report — the Geocaching
> tab bar it hardcoded had gained two tabs, so `tap_element` tapped the wrong
> one and reported success.
>
> Kept because its Problem Statement was right on the day it was written, and
> every one of its four predictions came true. Its open questions are live
> questions for the knowledge base, which did not exist when this was written:
> cache invalidation, the same identifier on different screens, rotation, and
> git-committable per-app files.
>
> Where the current thinking differs, and why:
>
> - This frames a coordinate cache as a **performance** mechanism with
>   validation attached. The failure it was meant to prevent turned out to be a
>   confident wrong answer rather than a slow one, so verification is the
>   point and speed is the side effect.
> - It aims at "zero configuration, transparent to test code and AI agents".
>   The knowledge base (#197) is deliberately the opposite: authored,
>   inspectable and diffable per screen, so an app moving shows up in review
>   instead of being silently re-learned.
> - The real answer to the 7-10s tree reads that motivated all of this was to
>   make the read cheap rather than to memorise its results — #196, and the
